...
public final class EventManager {
	 ...
	 * 
	 * Warning: This method should not be used. If you want to temporary unregister
	 * the listener, you should do like this:
	 * 
	 * <code>
	...
	/**
	 * Call an event and its parents, so the listeners listening it will get the
	 * event.
	 * 
	 * The event manager will stop dispatching cancellable events when cancelled,
	 * except if the listener ignore cancelled. The monitor listeners cannot cancel
	 * the event.
	 * 
	 * This method will call each parent's listeners, if they have one.
	 * 
	 * If an exception is caught while invoking the listener method, the exception
	 * will be logged.
	 * 
	 * @param event
	...
	private static void call(@NonNull Class<? extends Event> clazz, @NonNull Event event) {
		...
		for (ListenerMethod listener : map.values()) { // For each listener
			...
			if (!handler.ignoreCancelled() && event instanceof Cancellable && ((Cancellable) event).isCancelled()) { // Cancellable
																														// implementation
			...
			if (event instanceof Cancellable && handler.priority() == EventPriority.MONITOR) // If the listener is
																								// monitor
